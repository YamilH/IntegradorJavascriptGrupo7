...
function listarOradores() {/*f2*/
  //1 
  const respuesta = fetch(`http://localhost:8080/crud/api/orador`);
...
eliminarOrador = (id) => {
  ...
  fetch(`http://localhost:8080/crud/api/orador?id=${id}`, {
      method: "DELETE",
  })
...
const actualizarOrador = () => {
  ...
  fetch(`http://localhost:8080/crud/api/orador?id=${oradorSeleccionado.id}`, {
      method: "PUT",
      body: JSON.stringify(orador),
  })
